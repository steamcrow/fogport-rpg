"""Publish and verify Fogport's Civic Vigilance organization and main image."""
...
def validate_manifest(document: dict[str, Any]) -> tuple[Path, str]:
    ...
    if not image_path.is_file():
        raise SystemExit("Approved Blackcoat patrol image is missing.")
    actual_sha = hashlib.sha256(image_path.read_bytes()).hexdigest()
    if actual_sha != str(document["sha256"]).lower():
        raise SystemExit("Approved Blackcoat patrol image changed after approval.")
...
def main() -> None:
    ...
    if not fogport:
        raise SystemExit("Fogport location is missing; refusing to publish unlinked text.")
    fogport_link = f"[entity:{int(fogport['entity_id'])}|Fogport]"

    spec = document["organization"]
    entry = str(spec["entry"]).replace("{{FOGPORT_LINK}}", fogport_link)
    organization_path = f"campaigns/{CAMPAIGN_ID}/organisations"
    organizations = all_pages(token, organization_path)
    ...
    if match:
        ...
        request(
            token,
            "PATCH",
            f"{organization_path}/{organization_id}",
            payload=payload,
        )
        created = False
    else:
        made = request(
            token, "POST", organization_path, payload=payload
        ).get("data", {})
        organization_id = int(made["id"])
        created = True

    final = request(
        token, "GET", f"{organization_path}/{organization_id}"
    ).get("data", {})
    if (
        ...
        raise SystemExit("Civic Vigilance organization read-back failed.")
    ...
    if (
        ...
        raise SystemExit("Civic Vigilance image read-back failed.")

    receipt = {
        "published": True,
        "campaign": CAMPAIGN_NAME,
        "campaign_id": CAMPAIGN_ID,
        "organization": final["name"],
        ...
        "entry_verified": True,
        "fogport_link_verified": fogport_link in entry,
