"""Attach one checksum-locked main image to one exact Fogport note."""
...
def exact_one(records: list[dict[str, Any]], name: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise SystemExit(f"Expected exactly one note named {name!r}; found {len(matches)}.")
    return matches[0]
...
def validate(document: dict[str, Any]) -> tuple[Path, str]:
    ...
    if not path.is_file() or path.suffix.lower() not in ALLOWED_SUFFIXES:
        raise SystemExit("Approved note image is missing or invalid.")
    digest = hashlib.sha256(path.read_bytes()).hexdigest()
    if digest != str(document.get("sha256", "")).strip().lower():
        raise SystemExit("Approved note image changed after approval.")
    return path, digest
...
def upload(token: str, entity_id: int, image_path: Path) -> dict[str, Any]:
    ...
    with image_path.open("rb") as stream:
        response = requests.post(
            ...
            headers=headers(token), files={"file": (image_path.name, stream, mime_type)}, timeout=120,
        )
    if not response.ok:
        raise SystemExit(f"Kanka note image upload returned HTTP {response.status_code}: {response.text[:300]}")
    image = response.json().get("data", {}).get("image", {})
    if not isinstance(image, dict) or not image.get("uuid"):
        raise SystemExit("Kanka did not return note-image metadata.")
    return image
...
def main() -> None:
    ...
    note = exact_one(all_pages(token, f"campaigns/{CAMPAIGN_ID}/notes"), str(document["note_name"]))
    note_id, entity_id = int(note["id"]), int(note["entity_id"])
    before = request(token, "GET", f"campaigns/{CAMPAIGN_ID}/notes/{note_id}").get("data", {})
    ...
    if not isinstance(image, dict) or image.get("uuid") != uploaded.get("uuid") or not image.get("full") or not image.get("thumbnail"):
        raise SystemExit("Kanka note image read-back failed.")
    after = request(token, "GET", f"campaigns/{CAMPAIGN_ID}/notes/{note_id}").get("data", {})
    stable = ("id", "entity_id", "name", "type", "is_private", "entry")
    changed = [key for key in stable if before.get(key) != after.get(key)]
    if changed:
        raise SystemExit("Image-only boundary failed; non-image note fields changed: " + ", ".join(changed))
    receipt = {"published": True, "operation": "note-main-image-only", "campaign": CAMPAIGN_NAME, "campaign_id": CAMPAIGN_ID, "note": after["name"], "note_id": note_id, "entity_id": entity_id, "overview_url": f"https://app.kanka.io/w/{CAMPAIGN_ID}/entities/{entity_id}", "source_sha256": image_sha, "kanka_uuid": image["uuid"], "full": image["full"], "thumbnail": image["thumbnail"], "non_image_fields_changed": []}
