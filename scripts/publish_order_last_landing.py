"""Publish and verify the underground Order of the Last Landing in Fogport."""
...
def headers(token: str) -> dict[str, str]:
    return {"Authorization": f"Bearer {token}", "Accept": "application/json", "User-Agent": "Kanka-Librarian/1.0"}

def request(token: str, method: str, path: str, *, payload: dict[str, Any] | None = None,
            params: dict[str, Any] | None = None) -> dict[str, Any]:
    response = requests.request(method, f"https://api.kanka.io/1.0/{path}",
        headers={**headers(token), "Content-Type": "application/json"},
        json=payload, params=params, timeout=120)
    if not response.ok:
        raise SystemExit(f"Kanka {method} {path} returned HTTP {response.status_code}: {response.text[:500]}")
    body = response.json()
...
def all_pages(token: str, path: str) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        meta=body.get("meta",{})
        if page >= int(meta.get("last_page",page)): return records
        page += 1

def exact(records: list[dict[str, Any]], name: str, kind: str) -> dict[str, Any] | None:
    matches=[r for r in records if str(r.get("name","")).strip().casefold()==name.strip().casefold()]
    if len(matches)>1: raise SystemExit(f"Multiple {kind} records named {name!r}; refusing to guess.")
...
def validate_manifest(document: dict[str, Any]) -> tuple[Path, str]:
    ...
    if not image_path.is_file(): raise SystemExit("Approved Order of the Last Landing image is missing.")
...
def main() -> None:
    ...
    if (str(final.get("name"))!=payload["name"] or str(final.get("type"))!=payload["type"]
        ...
        raise SystemExit("Order of the Last Landing organization read-back failed.")
    ...
    if not isinstance(image_readback,dict) or image_readback.get("uuid")!=uploaded.get("uuid") or not image_readback.get("full") or not image_readback.get("thumbnail"):
        raise SystemExit("Order of the Last Landing image read-back failed.")
