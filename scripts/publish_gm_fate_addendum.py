...
def add_digest(d):
 u=deepcopy(d); u.pop("approval",None); return hashlib.sha256(json.dumps(u,ensure_ascii=False,sort_keys=True,separators=(",",":")).encode()).hexdigest()
def main():
 base=json.loads(BASE.read_text(encoding="utf-8")); verify_manifest(base); add=json.loads(ADD.read_text(encoding="utf-8"))
 if add.get("mode")!="gm-guide-section-addendum": raise SystemExit("Wrong addendum mode")
 if (int(add.get("campaign_id",0)),int(add.get("note_id",0)),int(add.get("entity_id",0)),int(add.get("post_id",0)))!=LOCKS: raise SystemExit("Addendum identity lock failed")
 ap=add.get("approval",{})
 ...
 for i,s in enumerate(sections):
  if s.get("heading")==add["insert_after"]:
   s["body"]+=add["danger_append"]; sections.insert(i+1,add["section"]); break
 ...
 with tempfile.TemporaryDirectory() as td:
  p=Path(td)/"merged.json"; p.write_text(json.dumps(base,ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); old=os.environ.get("MANIFEST_PATH"); os.environ["MANIFEST_PATH"]=str(p)
  try: publish_main()
  finally:
